...
**Commands** are instructions you type into the terminal to make the computer do something. A command usually starts with a word (the name of the command), followed by any options or information it needs. Each part of a command is separated by a space.
...
* `ls` is the command.
* `-l` is an option (options often start with a `-`).
* `/home` is extra information (an argument).

Commands are the core of interacting with the CLI. As you get comfortable with more commands, you'll unlock powerful ways to navigate and control your system.
